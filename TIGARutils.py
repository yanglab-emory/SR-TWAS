...
# def filter_weight_k_line(line: bytes, btarget: bytes, target_ind, col_inds):
# 	# split line into list
# 	row = line.split(b'\t')
# 	# check if row is for correct target
# 	if (row[target_ind].startswith(btarget)):
...
def tabix_query_file(path, reg_str):
		return len(subprocess.Popen(['tabix '+path+reg_str],
		shell=True, stdout=subprocess.PIPE,
		bufsize=1).stdout.readline()) > 0


def tabix_query_files(start, end, chrm, geno_path=None, gwas_path=None, w_path=None, z_path=None, w_paths=None, **kwargs):
	...
	if w_paths is None:
		...
		if not np.any(w_query):
			print('No cis-eQTL weights for target.')
			raise NoTargetDataError

		# for each weight path test if length of first line != 0
		target_ks = [i for i in range(len(w_query)) if w_query[i]]
		empty_target_ks = [i for i in range(len(w_query)) if not w_query[i]]
		
		return target_ks, empty_target_ks
...
def get_cols_dtype(file_cols, cols, sampleid=None, genofile_type=None, add_cols=[], drop_cols=[], add_dtype_dict={}, get_id=False, ind_namekey=False, ret_dict=False, **kwargs):
